...
if [[ "$MODE" != "--inside-allocation" ]]; then
    ...
    echo "Requesting a ${SLURM_TIME:-$DEFAULT_SLURM_TIME} interactive allocation on ${PARTITION:-$DEFAULT_PARTITION} with 2x V100 SXM2 nodes, 1 GPU per node..."
    salloc -p "${PARTITION:-$DEFAULT_PARTITION}" \
        --nodes=2 \
        --ntasks=2 \
        --ntasks-per-node=1 \
...
CTRL_OUT="$LOG_ROOT/qwen25_7b_2node_${JOBID}_${CONTROL_NODE}_${PORT_A}.out"
CTRL_ERR="$LOG_ROOT/qwen25_7b_2node_${JOBID}_${CONTROL_NODE}_${PORT_A}.err"
WORK_OUT="$LOG_ROOT/qwen25_7b_2node_${JOBID}_${WORKER_NODE}_${PORT_B}.out"
WORK_ERR="$LOG_ROOT/qwen25_7b_2node_${JOBID}_${WORKER_NODE}_${PORT_B}.err"
STATE_FILE="$HOME/.current_gpu_2node_qwen25coder7b_job"
...
echo "Starting Qwen2.5-Coder-7B server on worker node $WORKER_NODE at port $PORT_B..."
...
srun --nodes=1 --ntasks=1 --nodelist="$WORKER_NODE" \
     --gres=gpu:v100-sxm2:1 --cpus-per-task="${CPUS_PER_TASK:-8}" --mem="${SLURM_MEM:-32G}" --cpu-bind=none bash -lc '
    set -euo pipefail
...
srun --nodes=1 \
    ...
    echo "Starting Qwen2.5-Coder-7B server on control node $HA_CONTROL_NODE at port $HA_PORT_A..."
